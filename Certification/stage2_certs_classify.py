...
# -*- coding: utf-8 -*-
"""
阶段2 · A17 识别分类：原始件文件夹 → 已识别 / 未识别 → 原件归档
================================================================

流程：
  S1  列「待识别」文件夹全部文件（同名去重处理，同名重复件最后一起移送）
  S2  逐个下载 → 本地 A17 识别（process_one）
      - 识别成功 → 重命名 PDF（可多份）上传「已识别」文件夹（同名跳过）
      - 识别失败 → 原件上传「未识别」文件夹（同名跳过）
  S3  把「待识别」文件夹里的全部文件（含同名重复）移送「原始件归档」文件夹
  S4  机器人汇报（cert_common.notify）

文件夹（默认值，可参数/环境变量覆盖）：
  待识别   YIrbf0NzlloFNKdYAjvcL6gXnhe   ← 阶段1 产出
  已识别   VeXHfjiZqll255dgSOYcknbdnnf   ← 阶段3 输入
  未识别   Ic0Wf1PeelamrJd15GkcPo7Jnlb
  原件归档 HfYCfYMZnlhVpFdKI82cTHQ5npb
...
# 默认文件夹
# ============================================================
DEFAULT_RAW_FOLDER = "YIrbf0NzlloFNKdYAjvcL6gXnhe"      # 待识别（阶段1产出）
DEFAULT_OK_FOLDER = "VeXHfjiZqll255dgSOYcknbdnnf"       # 已识别（阶段3输入）
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--archive-folder",
                   ...
                   help=f"原件归档文件夹（默认 {DEFAULT_ARCHIVE_FOLDER}）")
...
def main() -> int:
    ...
    log(f"  待识别:   {args.raw_folder}", log_file)
    log(f"  已识别:   {args.ok_folder}", log_file)
    log(f"  未识别:   {args.bad_folder}", log_file)
    log(f"  原件归档: {args.archive_folder}", log_file)
    ...
    log(f"  去重后待识别 {len(unique)} 个（文件夹内同名重复 {dup_count} 个）", log_file)
    ...
    summary = {
        "ok_files": 0,       # 识别成功（上传到已识别的重命名 PDF 数）
        "ok_src": 0,         # 识别成功的源文件数
        "bad_src": 0,        # 识别失败（上传到未识别的原件数）
        "skip_ok_dup": 0,    # 已识别文件夹同名跳过
        ...
        if status == "ok":
            ...
            for p in outs:
                if p.name in ok_names:
                    ...
                    log(f"    ⏭ 已识别文件夹已有同名，跳过: {p.name}", log_file)
                    continue
                try:
                    upload_to_folder(token, args.ok_folder, p.name, p.read_bytes())
                    ok_names.add(p.name)
                    summary["ok_files"] += 1
                    log(f"    ✅ 上传已识别: {p.name}", log_file)
                except Exception as e:
                    summary["failed"] += 1
                    log(f"    ❌ 上传已识别失败 {p.name}: {e}", log_file)
    ...
    msg = (f"【质保书·②A17识别分类】完成 ✅\n"
           f"待识别 {len(unique)} 个（同名重复 {dup_count}）\n"
           ...
           f"已识别: {folder_url(args.ok_folder)}\n"
