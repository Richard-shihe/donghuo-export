...
# -*- coding: utf-8 -*-
"""
阶段1 · IEC 质保书 PDF 下载 → 上传「原件」飞书文件夹  （9.25_1 版）
======================================================

与 stage1_certs_download.py 的唯一区别：
  登录后新增【成员公司切换 SwitchGroup】——该账号下挂 27 家成员公司，
  质保书按「当前激活公司」隔离且全局存在服务端。不显式切换会随账号默认值
  漂移（2026 年中曾漂到华中公司 QL000000，导致近 3 月查空；业务主体是
  钢贸公司 QE000000，2541 条质保书持续更新）。本脚本每次登录后钉死到
  目标公司再查询。

流程：
  S0  IEC 登录（ibaosteel_client）+ 选组织 + 切换到目标成员公司
...
用法：
  python stage1_certs_download_9.25_1.py                       # 近3个月，最多200条
  python stage1_certs_download_9.25_1.py --preset today        # 交期=当月
  python stage1_certs_download_9.25_1.py --days 7 --max 50     # 近7天，最多50条
  python stage1_certs_download_9.25_1.py --max 100 --workers 6  # 提高并发下载速度（默认4）
  python stage1_certs_download_9.25_1.py --date-from 202606 --date-to 202608
  python stage1_certs_download_9.25_1.py --dry-run             # 登录+查询+decrypt，不下载不上传
...
环境变量：
  ...
  IEC_MEMBER_COMPANY_CODE              目标成员公司代码（默认 QE000000 钢贸公司）
"""
...
# showRsaCertificate 9 参数固定值（渠道销售 · 钢贸公司 QE000000）
# 注意：这些值必须与「当前激活的成员公司」一致——切到别的公司后页面
# 会渲染该公司的 segNo/companyNo/saleNetwork，下载参数也要跟着改。
FORM_FIELDS = {
    "uuCode": "U41634",
    "userNum": "062122",
    "saleNetWork": "E",
...
# 目标成员公司（质保书按激活公司隔离，见模块 docstring）
DEFAULT_MEMBER_COMPANY = "QE000000"  # 钢贸公司
...
def switch_member_company(s: requests.Session, target_code: str = "") -> None:
    """把账号激活的公司切到目标成员公司（质保书按「当前激活公司」隔离，全局存在服务端）。

    target_code 为空时取环境变量 IEC_MEMBER_COMPANY_CODE，再回退默认 QE000000（钢贸公司）。
    切换失败直接抛错——宁可失败也不要静默查错公司。"""
    code = target_code or env("IEC_MEMBER_COMPANY_CODE", DEFAULT_MEMBER_COMPANY)

    r = s.post(f"{BASE_HTTPS}/iecs/operation/switchGroup/switchGroup/querySwitchGroup",
               timeout=20)
    d = r.json()
    if str(d.get("code", "")) != "0":
        raise RuntimeError(f"querySwitchGroup 失败: {d}")
    companies = d.get("data") or []
    if not companies:
        raise RuntimeError("querySwitchGroup 返回空公司列表")

    target = next((c for c in companies if c.get("memberCompanyCode") == code), None)
    if not target:
        names = "、".join(f'{c.get("memberCompanyName")}({c.get("memberCompanyCode")})'
                          for c in companies)
        raise RuntimeError(f"成员公司列表里没有 {code}。可选（{len(companies)} 家）: {names}")
    name = str(target.get("memberCompanyName") or "")

    r2 = s.post(
        f"{BASE_HTTPS}/iecs/operation/switchGroup/switchGroup/editSwitchGroup",
        json={"memberCompanyCode": code, "memberCompanyName": name},
        timeout=20,
    )
    d2 = r2.json()
    if str(d2.get("code", "")) != "0":
        raise RuntimeError(f"切换到 {name}({code}) 失败: {d2}")
    print(f"  已切换到成员公司: {name} ({code})，账号下共 {len(companies)} 家可选", flush=True)
...
def login_and_session() -> Tuple[IEC, requests.Session, str]:
    ...
    # 9.25 新增：显式切到目标成员公司，防止账号默认激活公司漂移导致查空
    switch_member_company(s)
    return iec, s, token
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="阶段1 · IEC 质保书 PDF 下载 → 上传「原件」飞书文件夹（9.25_1 版：登录后切换成员公司）",
...
def main() -> int:
    ...
    log("========== 【阶段1】IEC 质保书下载 → 原件文件夹（9.25_1）==========", log_file)
    ...
    log(f"  目标成员公司: {env('IEC_MEMBER_COMPANY_CODE', DEFAULT_MEMBER_COMPANY)}", log_file)
    log(f"  待处理文件夹: {args.raw_folder}", log_file)
    log(f"  dry_run: {args.dry_run}", log_file)

    # S0 登录 + 切换成员公司
    log("\n[S0] IEC 登录 + 选组织 + 切换成员公司", log_file)
    try:
