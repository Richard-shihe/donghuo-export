...
默认筛选（与 download_bundle.py / export_lindiao.py 风格一致）：
  出厂日期: 近 30 天 ~ 今天（= 昨天前 29 天到今天，共 30 天）
...
命令行：
  python export_iec_weightmemo_bundle.py                     # 出厂日期近 30 天，自动上传
  python export_iec_weightmemo_bundle.py --days 90          # 近 90 天
  python export_iec_weightmemo_bundle.py --date-start 20260801 --date-end 20260821
  python export_iec_weightmemo_bundle.py --dry-run          # 只下载不上传
  python export_iec_weightmemo_bundle.py --no-upload        # 同上（等价 DRY_RUN=1）
"""
...
def _default_date_range(days: int) -> Tuple[str, str]:
    ...
    days=30 → [today - 29 days, today] 共 30 天。
    """
...
def parse_args():
    ...
    p.add_argument("--days", type=int, default=30,
                   help="出厂日期范围：近 N 天（含今日，默认 30）。被 --date-start/end 覆盖。")
